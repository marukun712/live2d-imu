...
- `👻` が末端レイヤー（PSD上で非表示）= `walkPSD` では読み込まれるため、不要なら `SKIP` に追加する
...
```ts
rigNodes(nodes, {
  head:      pipe(psdGroup("顔グループ"), psdGroup("耳グループ")),
...
- 表情差分をすべて含めてしまう (同時に複数のエフェクトや表情がかかってしまう) → もっともベーシックなもののみ含め、差分はSKIPへ
