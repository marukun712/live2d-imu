---
name: KokoroRig
description: KokoroRigでリギングをする方法
---
...
```ts
type RigMap   = Partial<Record<BONE_NAME, GroupMatcher>>  // rigNodes に渡す
type GroupMap = Partial<Record<FACE_NAME, GroupMatcher>>  // groupNodes に渡す
```

- `RigMap` / `GroupMap` はどちらも **`Partial`** なので、**使うキーだけ書けばよい**
- 使わないボーン・パーツのキーは省略する
...
### 目的
`SKIP` に指定した名前のレイヤー（またはグループ）は `walkPSD` で読み込まれず、描画・変形の対象から外れる。
**用途はPSDごとに異なる**ため、ツリーを見て判断する。

### 仕様
`walkPSD` の内部実装は以下のように動作する。

```ts
if (!layer.name || layer.hidden || skip?.has(layer.name)) return;
```

グループ名を `SKIP` に入れると、その時点で `return` するため**子レイヤーごとすべてスキップ**される。
個別のレイヤー名を指定することも可能。
...
### ルール
1. **使うボーンのキーだけ書けばよい**（`Partial` なので省略可）
2. 各ボーンには、変形させたいレイヤー群にマッチするマッチャーを設定する
3. 複数グループをまとめる場合は `pipe()` でOR結合する
4. 除外したい子グループがある場合は `psdGroup(group, ["除外グループ名"])` を使う
...
- `body` の一部である袖・襟などが独立レイヤーの場合、`byName()` や `pipe()` で個別に拾う
